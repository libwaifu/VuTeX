 ...
 * @source: https://github.com/surmon-china/simple-netease-cloud-music/blob/master/src/netease.js
 */
...
export class Netease {
     ...
     * @return {Promise}
     */
    search(keyword: string, page = 1, limit = 10) {
        const body = {
            method: 'POST',
            params: {
                s: keyword,
 ...
 */
    ...
    picture(id:number|string, size = 300) {
        ...
        const netease_pickey = id => {
            ...
            const song_id = id.split('').map((item, index) => {
                return String.fromCharCode(item.charCodeAt() ^ (magic[index % magic.length]).charCodeAt(0))
            })
     ...
     * @return Promise
     */
    private makeRequest(options, form: string) {
    ...
    /*
    private doRequest(options, form: string) {
        let postData = querystring.stringify({});
        let request = http.request(options, (res) => {
            //console.log(`STATUS: ${res.statusCode}`);
            //console.log(`HEADERS: ${JSON.stringify(res.headers)}`);
            res.on('data', (chunk:Buffer) => {return `${chunk}`});
            res.on('end', () => {console.log('No more data in response.')})
          });
          if (form) {request.write(form)}
    ...
    */
     ...
     */
    private onResponse(response, resolve, reject) {
